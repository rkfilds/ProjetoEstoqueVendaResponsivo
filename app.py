...
def index():
    ...
    if termo_busca:
        busca = f"%{termo_busca}%"
        query = query.filter(
            Produto.produto.ilike(busca) |
            Produto.marca.ilike(busca) |
            Produto.modelo.ilike(busca) |
            Produto.material.ilike(busca) |
...
def listar_produtos():
    ...
    if produto:
        query = query.filter(Produto.produto.ilike(f'%{produto}%'))
    if marca:
        query = query.filter(Produto.marca.ilike(f'%{marca}%'))
    if material:
        query = query.filter(Produto.material.ilike(f'%{material}%'))
    if cor:
...
with app.app_context():
    db.create_all()  # Executado no Render e localmente
